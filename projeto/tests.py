...
class TestesAPIListarProjetos(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='cliente', password='12345')
        self.perfil = Perfil.objects.create(
            usuario=self.user,
    ...
    def test_get_ativos(self):
        ...
        self.assertEqual(data[0]['titulo'], 'Projeto Pendente')
    ...
    def test_get_concluidos(self):
        ...
        data = json.loads(response.content)
        self.assertEqual(len(data), 1)
        self.assertEqual(data[0]['titulo'], 'Projeto Concluído')
        self.assertEqual(data[0]['nome_habilidades'], ['Design Gráfico'])
        self.assertEqual(data[0]['nome_status'], 'CONCLUÍDO')